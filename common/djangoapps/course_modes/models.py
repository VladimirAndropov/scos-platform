...
class CourseMode(models.Model):
    ...
    # use "honor"
    DEFAULT_SHOPPINGCART_MODE_SLUG = VERIFIED
    DEFAULT_SHOPPINGCART_MODE = Mode(VERIFIED, _('Verified'), 0, '', 'usd', None, None, None, None)
...
#            raise ValidationError(_(u"Verified modes cannot be free."))
    ...
    def can_auto_enroll(cls, course_id, modes_dict=None):
        ...
        if cls.is_verified_free(course_id, modes_dict=modes_dict):
            return True
    ...
    @classmethod
    def is_verified_free(cls, course_id, modes_dict=None):

        if modes_dict is None:
            modes_dict = cls.modes_for_course_dict(course_id)

        if cls.VERIFIED in modes_dict and modes_dict["verified"].min_price == 0:
            return True
        return False
    ...
    def auto_enroll_mode(cls, course_id, modes_dict=None):
        ...
        if cls.HONOR in modes_dict:
            ...
            return cls.AUDIT
        elif cls.VERIFIED in modes_dict:
            return cls.VERIFIED
