...
class CourseEnrollmentAdminTest(SharedModuleStoreTestCase):
    ...
    def test_username_exact_match(self):
        ...
        CourseEnrollmentFactory(user=user2, course_id=self.course.id,) # pylint: disable=no-member
    ...
    def test_save_toggle_active(self):
        ...
        with COURSE_ENROLLMENT_ADMIN_SWITCH.override(active=True):
            response = self.client.post(
                reverse('admin:student_courseenrollment_change', args=(self.course_enrollment.id,)), data=data,
            )
    ...
    def test_save_invalid_course_id(self):
        ...
        """
        data = {'user': unicode(self.course_enrollment.user.id), 'course': 'invalid-course-id', 'is_active': 'true',
            'mode': self.course_enrollment.mode, }
