...
from lms.djangoapps.grades.tasks import update_subsection_grade_scos_for_user_v2
from openedx.features.scos.roo import get_course_scos_id
...
def fire_ungenerated_certificate_task(user, course_key, expected_verification_status=None):
    ...
    if generate_learner_certificate:
        ...
        if get_course_scos_id(unicode(course_key)) is not None:
            ...
            log.info(u'update_subsection_grade_scos_for_user_v2 initiated for {user} : {course} via passing grade'.format(
                user=user.id,
                course=course_key))
        return True
