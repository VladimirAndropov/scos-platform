    ...
    * How the user reached the page (mostly affects messaging)
    ...
    def get(
        ...
        if relevant_course_mode is not None:
            ...
            # raise Http404
            CourseEnrollment.enroll_by_email(request.user.email, course_key, CourseMode.VERIFIED)
            ...
            if 'action' in request.GET:
                if request.GET['action'] == 'confirm':
                    SoftwareSecurePhotoVerification.objects.get_or_create(
                        ...
                        copy_id_photo_from_id=request.user,
                        photo_id_key='1',
                        face_image_url='',
                        photo_id_image_url='',
                        submitted_at=datetime.date.today(),
                        user=request.user,
                        status='approved',
                        created_at__gte=datetime.date.today(),
                        defaults={'name': request.user.profile.name},
                        )
